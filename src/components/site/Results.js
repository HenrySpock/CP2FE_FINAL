...
function Results() {
  ...
  useEffect(() => {
    ...
    async function fetchUsers() {
      try {
        const response = await fetch(`http://localhost:5000/search/users?username=${encodeURIComponent(searchTerm)}&currentUserId=${user.user_id}`);
        if (!response.ok) {
    ...
    async function fetchTrips() {
      try {
        const response = await fetch(`http://localhost:5000/search/trips?title=${encodeURIComponent(searchTerm)}&tripEntry=${encodeURIComponent(searchTerm)}&currentUserId=${user.user_id}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
    ...
    async function fetchTravelogs() {
      try {
        const response = await fetch(`http://localhost:5000/search/travelogs?searchTerm=${encodeURIComponent(searchTerm)}&currentUserId=${user.user_id}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
    ...
    async function fetchTravelogImages() {
      try {
        const response = await fetch(`http://localhost:5000/search/images/travelogs?searchTerm=${encodeURIComponent(searchTerm)}&currentUserId=${user.user_id}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
