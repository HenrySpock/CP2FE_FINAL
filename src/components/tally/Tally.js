    ...
    const fetchCounts = async () => {
      if (isAuthenticated && user) {
        try {
          ...
          const userNotificationsResponse = await fetch(`http://localhost:5000/tally/unread-user-notifications?userId=${user.user_id}`, {
            method: 'GET', 
          });
 
          const userMessagesResponse = await fetch(`http://localhost:5000/tally/unread-user-messages?userId=${user.user_id}&isAdmin=${user.isAdmin}`, {
            method: 'GET',
          });

          const adminMessagesResponse = await fetch(`http://localhost:5000/tally/unread-admin-messages?userId=${user.user_id}`, {
            method: 'GET',
          });
          
          const adminReportsResponse = await fetch(`http://localhost:5000/tally/unread-admin-reports`, {
            method: 'GET',
          });
          ...
          const checkUserStatus = async () => {
            if (user && isAuthenticated) {
              ...
              const maintenanceResponse = await fetch('http://localhost:5000/api/maintenance/status');
              ...
              const suspensionResponse = await fetch(`http://localhost:5000/feedback/api/check-suspension?userEmail=${encodeURIComponent(user.email)}`);
              ...
              const bannedEmailResponse = await fetch(`http://localhost:5000/feedback/api/check-banned-email?email=${encodeURIComponent(user.email)}`);
