...
function TripDet() { 
  ...
  useEffect(() => {
    const fetchUserDataByUsername = async (username) => {
      try {
        const response = await axios.get(`http://localhost:5000/user/api/users/${username}`);
    ...
    const checkUserAndPermissions = async () => {
      if (trip_id) {
        try {
          ...
          const response = await axios.get(`http://localhost:5000/trip/api/tripgetnotprivate/${trip_id}`);
          if (response.data) {
      ...
      if (user && trip_id) {
        try {
          ...
          const authorResponse = await axios.get(`http://localhost:5000/trip/api/tripget/${trip_id}`, { params: { userId: user.user_id } });
      ...
      if (user && user.user_id && trip_id) {
        try {
          ...
          const permissionResponse = await axios.get(`http://localhost:5000/permissions/check`, { params: { tripId: trip_id, granteeId: user.user_id } });
  ...
  useEffect(() => {
    ...
    const incrementViewCount = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/viewcount/api/trip/increment-view-count/${trip_id}`, { method: 'PATCH' });
    ...
    if (user) {
      const fetchTrip = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/trip/api/tripdet/${trip_id}`);
  ...
  useEffect(() => {
    const fetchTravelogs = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/trip/api/travelogs/${trip_id}`); 
  ...
  useEffect(() => {
    const fetchUserTravelogs = async () => { 
      ...
      try {
        const response = await axios.get(`http://localhost:5000/travelog/api/user/${user.user_id}/travelogs`);
  ...
  const handleUpdate = async (event) => { 
    ...
    try {
      ...
      await axios.patch(`http://localhost:5000/trip/api/trips/${trip_id}`, editedTrip);
      ...
      const addPromises = travelogsToAdd.map(travelog =>
        axios.patch(`http://localhost:5000/travelog/api/travelog/${travelog.travelogId}`, { tripId: trip_id, user_id: user.user_id })
      );
  
      // Send requests to update travelogs that have been deselected
      const removePromises = travelogsToRemove.map(travelog =>
        axios.patch(`http://localhost:5000/travelog/api/travelog/${travelog.travelogId}`, { tripId: null, user_id: user.user_id })
      );
  ...
  const cancelEdit = () => {
    ...
    const handleDelete = async () => {
      ...
      try {
        ...
        await axios.delete(`http://localhost:5000/trip/api/trips/${trip_id}?user_id=${user_id}`);
  ...
  const submitReport = async (complaint) => {
    ...
    try { 
      ...
      const response = await fetch(`http://localhost:5000/feedback/api/trip/${trip_id}/report`, { 
