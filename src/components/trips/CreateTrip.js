...
const updateImageUrl = (earliestTravelog) => {
  ...
  useEffect(() => {
    const fetchTravelogs = async () => {
      if (user) {
        try {
          const response = await fetch(`http://localhost:5000/trip/api/travelogs?userId=${user.user_id}&tripId=null`);
          if (!response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/trip/api/trips', {
        method: 'POST',
        headers: {
      ...
      for (const travelog of selectedTravelogs) {
        ...
        try {
          ...
          const travelogResponse = await fetch(`http://localhost:5000/travelog/api/travelog/${travelogId}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
