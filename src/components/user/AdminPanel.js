    ...
    if (user) {
      const intervalId = setInterval(async () => {
        ...
        for (const report of reportedFeedback) {
          ...
          if (otherUserId) {
            const response = await fetch(`http://localhost:5000/tally/unread-messages-count?adminUserId=${user.user_id}&otherUserId=${otherUserId}`);
            if(response.ok) {
...
// console.log('user.user_id: ', user.user_id)
  ...
  useEffect(() => {
    async function fetchReportedFeedback() {
      try {
        const response = await axios.get('http://localhost:5000/feedback/api/reported-feedback');
        ...
        const suspensionChecks = reportedFeedbackData.map(async (report) => {
          ...
          if (userEmail) {
            const suspensionResponse = await axios.get(`http://localhost:5000/feedback/api/check-suspension?userEmail=${userEmail}`);
  ...
  const handleClearReport = async (reportId, index) => {
    try {
      const response = await axios.patch(`http://localhost:5000/feedback/api/clear-report/${reportId}/clear`, {
        cleared: true,
    ...
    async function checkSuspensions() {
      ...
      reportedFeedback.forEach(async (report) => {
        if (report.ReportedUser) {
          const response = await axios.get(`http://localhost:5000/feedback/api/check-suspension?userEmail=${report.ReportedUser.email}`);
  ...
  const toggleSuspension = async (userEmail) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/feedback/api/suspend', {
        userEmail,
        action,
      });
  ...
  const handleBanUser = async (userId, email) => {
    ...
    try {
      ...
      const banResponse = await fetch('http://localhost:5000/feedback/ban-email', {
        method: 'POST',
      ...
      if (banResponse.ok) {
        ...
        const deleteUserResponse = await fetch(`http://localhost:5000/user/api/user/${userId}`, {
          method: 'DELETE',
        });
  ...
  const handleNotifyUser = async (userId) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/notify-user', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
