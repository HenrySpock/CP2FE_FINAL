...
function Permissions() { 
  ...
  useEffect(() => {
    const fetchFriendsAndPermissions = async () => {
      try {
        const friendsResponse = await axios.get(`http://localhost:5000/api/friends/${user.user_id}`);
        ...
        for (const friend of friendsList) {
          ...
          const permissionResponse = await axios.get(`http://localhost:5000/api/permissions/specific/${friend.user_id}`, {
  ...
  const handleGrant = async () => {
    ...
    try {
      await axios.patch('http://localhost:5000/permissions/update', updatedPermissions);
