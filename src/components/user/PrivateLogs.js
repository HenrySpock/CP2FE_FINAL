    ...
    if (user && user.user_id.toString() === user_id) { 
      const fetchPrivateData = async () => {
        ...
        try {
          ...
          const tripsResponse = await axios.get(`http://localhost:5000/permissions/trips/${user_id}`);
          const travelogsResponse = await axios.get(`http://localhost:5000/permissions/travelogs/${user_id}`);
