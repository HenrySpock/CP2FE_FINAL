...
function TravDet() {
  ...
  const handleDeleteTravelog = async () => { 
    try {
      ...
      await axios.delete(`http://localhost:5000/travelog/api/travelog/${travelogId}?user_id=${userId}`);
      navigate('/hub'); 
    } catch (error) {
  ...
  const submitReport = async (complaint) => {
    try { 
      ...
      const response = await fetch(`http://localhost:5000/feedback/api/travelog/${travelogId}/report`, {
        method: 'POST', 
        headers: {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      if (travelog && currentUser) { 
        try {
          // Check block status
          const blockResponse = await fetch(`http://localhost:5000/travelog/api/users/${travelog.user_id}/block-status/${currentUser.user_id}`);
          ...
          if (travelog.isPrivate) {
            const permissionUrl = `http://localhost:5000/permissions/check?travelogId=${travelog.travelogId}&granteeId=${currentUser.user_id}`;
  ...
  const fetchTravelog = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/travelog/api/travelog/${travelogId}`);
  ...
  const incrementViewCount = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/viewcount/api/travelog/increment-view-count/${travelogId}`, {
        method: 'PATCH', 
      });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // const response = await axios.patch(`http://localhost:5000/travelog/api/travelog/${travelogId}`, editedTravelog);
      await axios.patch(`http://localhost:5000/travelog/api/travelog/${travelogId}`, editedTravelog);
