...
function TravDetImageEdit({ user, travelogId, fetchTravelog, initialImages, username ='', profileUser, userData }) {
  ...
  const openFullscreenModal = (imageUrl, imageId, title, description) => {
    ...
    const incrementImageViewCount = async (image_id) => {
        try {
          const response = await fetch(`http://localhost:5000/viewcount/api/image/increment-view-count/${image_id}`, {
            method: 'PATCH',
          });
    ...
    const saveImages = async () => {
      try {
          ...
          if (deletedImageIds.length > 0) {
              await axios.post(`http://localhost:5000/travelog/api/travelog/${travelogId}/delete-images`, {
          ...
          const response = await axios.patch(`http://localhost:5000/travelog/api/travelog/${travelogId}/images`, {
              images: images,
              user_id: user.user_id,
              imageInfo: imageInfo,
          });
