...
useEffect(() => {
  ...
  useEffect(() => { 
    const fetchLikeStatus = async () => {
      if (userData && contextUser && travelog_id) {
        try {
          const response = await fetch(`https://lgcbe.onrender.com/api/likes/travelog/check?user_id=${userData.user_id}&liker_id=${contextUser.user_id}&travelog_id=${travelog_id}`);
          if (!response.ok) {
  ...
  const toggleLike = async (user_id, liker_id, liketype, travelog_id) => { 
    ...
    try {
      const response = await fetch('https://lgcbe.onrender.com/api/likes/travelog', {
        method: 'POST',
        headers: {
